ggGenePlot_regLine_xcount_auto <- function(gene,all_data, normBy, prefix, myWidth, myHeight){
  #all_data is dataframe of normalized counts with associated metadata; rows are samples and columns are genes/metadata
  all_data[,"X_count"] <- as.numeric(all_data[,"X_count"])
  all_data[,gene] <- as.numeric(all_data[,gene] /normBy)
  
  
  data <- all_data[,c("X_count", gene)]
  colnames(data) <- c("X_count", "gene_expression")
  pdf(file=paste0(myPath, "/Figures/",prefix,"_",gene,"_linearPlot.pdf"), width=myWidth, height=myHeight)
  p <- ggplot(data = data, aes(x = X_count, y = gene_expression)) +
