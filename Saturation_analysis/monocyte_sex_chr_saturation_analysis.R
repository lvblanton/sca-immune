...
#that respond to Chr X or Chr Y dosage in monocytes. 
...
expressedGenes_mono_sexlinked <- mono_expressed$expSexChromGenes

#bring in counts
normCounts_expXgenes <- read.delim(file="/sca-immune/Sex_chromosomal_linear_regressions/monocyte/mono_normCounts_expXgenes.txt", stringsAsFactors = FALSE, check.names = FALSE)
...
for(size in c(1:length(SAMPLE.SIZE))){
  ...
  repeat{
    ...
    mono.metadata.random <- metadata.mono[metadata.mono$SampleID %in% colnames(normCounts_expXgenes.random),]
    mono.metadata.random <- mono.metadata.random[order(rownames(mono.metadata.random)),]
    sample_table_par_npx <- mono.metadata.random[,c("X_count", "Y_count", "GTC_Run_ID")]
  ...
  write.table(saturationResults, file=paste0("/sca-immune/Saturation_analysis/monocyte_sex_chr_output/monocyte_saturationResults_100iterations_", as.character(i),".txt"), quote=FALSE, col.names = TRUE, row.names = FALSE, sep="\t")
